...
test("sanitizeProviderUsageOutput redacts account identity before browser parsing", () => {
  const source = JSON.stringify({
    ...
    accountsWithoutUsage: [{
      ...
      accountId: "other-secret-id",
    }],
    disabledCredentials: [{
      provider: "openai-codex",
      email: "disabled@example.com",
      accountId: "disabled-secret-id",
    }],
  });
  ...
  assert.equal(sanitized.includes("other@example.com"), false);
  assert.equal(sanitized.includes("other-secret-id"), false);
  assert.equal(sanitized.includes("disabled@example.com"), false);
  assert.equal(sanitized.includes("disabled-secret-id"), false);
